...
- XC8 Compiler [2.40 or newer](https://www.microchip.com/mplab/compilers)
...
### Sampling of OPA output using ADCC
The output of OPA is fed the ADCC internally. ADCC count is then used for calculation of percentage of maximum force applied.

### Force calculation
As per the specifications of the FSR (Refer [FSR Datasheet](https://www.interlinkelectronics.com/fsr-400-series)), ideally a maximum of 10N which corresponds to 1kg can be applied to the FSR. When no force is applied , FSR resistance is greater than 10 MΩ and output voltage of OPA is 1.65V *(Refer equation 1)*. When maximum force is applied, FSR resistance is around 1.25kΩ and the corresponding OPA output voltage is calculated as 3.63 V. As the OPA output is limited to 3.3V in this implementation, corresponding resistance of FSR is calculated approximately as 1.5kΩ. Refer equation below.

`RFSR = RG/((2 * VOUT/VDD) – 1)   -----------------------------------------(2)`

Therefore, the maximum force that can be applied and sensed using this implementation is approximately limited to 700g (corresponding to 1.5kΩ FSR resistance).
...
6.	Add new variables for ADCC count (Uint16) and Force Applied (Uint8) as shown in image below and Click on ‘Save’.
...
|---------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------|
|    Clock Control    |    Clock source –   HFINTOSC<br>HF Internal Clock – 4MHz<br>Clock Divider   – 1                                                                                                                                                                                                                                                                                                         |    System  clock                                                                     |
